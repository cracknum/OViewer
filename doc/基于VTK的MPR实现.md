...
为了减少纹理的重复创建、销毁、上传所带来的性能损耗，尝试使用一个大纹理和cuda绑定再cuda中进行切片数据写入，在OpenGL中对纹理进行采样绘制。

最大纹理尺寸计算方式
$$
N = max(\sqrt{W^2+H^2}, \sqrt{W^2+D^2}, \sqrt{D^2+H^2})
$$

最大纹理尺寸为$N \times N$,其中W、H、D分别为体数据的宽、高和深度
虽然并不是很理解这个公式，但是先用着